...
<?php 
	...
	function insert_post_gallery($post_gallery){
		$wp_error = '';
		$post_id = wp_insert_post( $post_gallery, $wp_error);


		$guid = "http://local.osc-pro.com/?post_type=gallery&#038;p=".$post_id;
		$post_gallery['ID'] = $post_id;
		$post_gallery['guid'] = $guid;
	...
	function insert_product_image($my_post,$coname,$org_post_id){
		...
		add_post_meta($post_id,'gllr_image_text',$my_post['post_content']);
		...
		// update osg post
		$post_info = get_post($org_post_id,ARRAY_A);

		// 如果没插入过  那么插入相册
		if ( stripos($post_info['post_content'], 'print_gllr')) {
			# code...
		}else{
			$post_info['post_content'] = $post_info['post_content']."<br>[print_gllr id=".$my_post['post_parent']."]";
	...
	function movefile($target_path,$target, $source,$name,$year,$month,$gallery_id)
	{
		...
		echo "<br>=========<br>";
	    	...
	    	$res['sizes']['photo-thumb']    = resize_save(160,120,$target,$target_path,$name,false);
...
<?php 
	...
	$result = $wpdb->get_results( 
		" SELECT g.img_id, g.article_id,g.img_original,g.img_desc, p.ID, p.post_title,p.post_date FROM sm_articles_gallery as g ".
		" LEFT JOIN wp_posts as p on p.menu_order = g.article_id ".
		" LEFT JOIN sm_article as a on a.article_id = g.article_id ".
		// ." WHERE g.img_id < 200 "
		" WHERE p.post_type like 'post' AND a.cat_id = 13 group by article_id "

	 	,ARRAY_A);
	 */
	/**/
	$result = $wpdb->get_results( 
		" SELECT g.img_id, g.article_id,g.img_original,g.img_desc, p.ID, p.post_title,p.post_date,p.post_name FROM sm_articles_gallery as g ".
		" LEFT JOIN wp_posts as p on p.menu_order = g.article_id ".
		" LEFT JOIN sm_article as a on a.article_id = g.article_id ".
		// ." WHERE g.img_id < 200 "
		" WHERE p.post_type like 'gallery' AND ( a.cat_id = 13 OR  a.cat_id = 12) "
		// ." AND g.article_id = 180 "
	 	,ARRAY_A);
	...
	// print_r($sm_brand);
	   	...
	   	foreach ($result as $key => $value) {
			$org_post_id = str_replace( 'gallery', '',$value['post_name']);

			echo "img_id ".$value['img_id']." gid ".$value['article_id']."- pid.".$value['ID']." - org_post_id ".$org_post_id ."-".$value['post_title'].'-'.$value['img_original']."<br>"; //
			...
			/*
			$gallery_post = array(
					'post_title' => $value['post_title']."相册", 
					'post_name'	 => 'gallery'.$value['ID'],
			...
			// insert_post_gallery($gallery_post);
			...
			*/
			...
			if ( ! is_wp_error( $image ) && !empty($value['img_original'])){
				echo "<br>".$_source." OKOKOK<br>";

				// $gallery_id = insert_product_image($my_post,$coname,$org_post_id);
				// movefile($_path,$_target,$_source,$name,$year,$month,$gallery_id);

			}else{
				echo "<br>".$_source." breaken<br>";
			}
